...
class StunProtocol(DatagramProtocol, object):
    ...
    def datagramReceived(self, dgram, address):
        ...
        Calls the methods for message parsing and analyse
        """
        try:
            self.stopTimeout()
        except:
    ...
    def send(self, to, avpairs=()):
        """
        Packs the response and sends it to the client
        """
...
class StunServer(StunProtocol, object):
    ...
    def createResponse(self, toAddr, listAttr):
        """Prepares the response and send it to the client"""
        avpairs = ()
...
#        self.configuration[2] = ('127.0.0.1', self.listenPort)
    ...
    def _timeout(self):
        """Called if a timeout is expired
        Send 9 message at:
    ...
    def retransmissionEnded(self):
        """Transaction have failed (called after timeout retrasmissions at 9500ms)"""
